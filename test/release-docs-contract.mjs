...
console.log('RELEASE DOCS 1/1: public version sources agree on 1.0.1')
...
for (const [label, source] of [['English README', readme], ['Chinese README', readmeZh]]) {
  ...
  assert.ok(harnessWeb?.[0].includes('0.1.2-rc.1'), `${label} states the Harness Web baseline`)
...
const releaseNotesPath = 'docs/releases/1.0.1.md'
...
assert.match(releaseNotes, /^# dsh-deepread 1\.0\.1/m)
assert.match(releaseNotes, /issue #10[\s\S]*dsh-client-store/i)
...
assert.match(releaseNotes, /1\.0\.0[\s\S]*0\.5\.4/i)
...
assert.match(readme, /\[Release notes\]\(docs\/releases\/1\.0\.1\.md\)/)
assert.match(readmeZh, /\[正式版说明\]\(docs\/releases\/1\.0\.1\.md\)/)
